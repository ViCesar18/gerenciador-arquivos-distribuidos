...
def listar_arquivos():
    ...
    with xmlrpc.client.ServerProxy("http://localhost:8200/") as proxy:
        arquivos1 = proxy.listar_arquivos_middle()

    with xmlrpc.client.ServerProxy("http://localhost:8300/") as proxy:
        arquivos2 = proxy.listar_arquivos_middle()

    with xmlrpc.client.ServerProxy("http://localhost:8400/") as proxy:
        arquivos3 = proxy.listar_arquivos_middle()

    return Union(os.listdir("./dir"), arquivos1, arquivos2, arquivos3)
